...
test("help returns command list as JSON", () => {
  ...
  assert.ok(output.commands.includes("agentshell package deps [--compact]"));
  assert.ok(output.commands.includes("agentshell files changed [--compact]"));
  assert.ok(output.commands.includes("agentshell file info <path> [--compact]"));
  assert.ok(output.commands.includes("agentshell test list [--compact] [--max-files N]"));
  assert.ok(output.commands.includes("agentshell errors from-log <file> [--compact]"));
  assert.ok(output.commands.includes("agentshell imports <file> [--compact]"));
  assert.ok(output.commands.includes("agentshell symbols <file> [--compact] [--max-symbols N]"));
  assert.ok(output.commands.includes("agentshell refs <symbol> [--compact] [--limit N]"));
  assert.ok(output.commands.includes("agentshell config list [--compact] [--max-configs N]"));
  assert.ok(output.commands.includes("agentshell git status [--compact] [--max-files N]"));
  assert.ok(output.commands.includes("agentshell git diff [--compact] [--staged]"));
  assert.ok(output.commands.includes("agentshell git log [--compact] [--limit N]"));
  assert.ok(output.commands.includes("agentshell git branch [--compact] [--max-branches N]"));
...
test("node src/cli.js manual --full preserves complete command map", () => {
  ...
  assert.ok(output.commandMap.some((entry) => entry.command === "agentshell files changed [--compact]"));
  assert.ok(output.commandMap.some((entry) => entry.command === "agentshell file info <path> [--compact]"));
  assert.ok(output.commandMap.some((entry) => entry.command === "agentshell test list [--compact] [--max-files N]"));
  assert.ok(output.commandMap.some((entry) => entry.command === "agentshell errors from-log <file> [--compact]"));
  assert.ok(output.commandMap.some((entry) => entry.command === "agentshell imports <file> [--compact]"));
  assert.ok(output.commandMap.some((entry) => entry.command === "agentshell symbols <file> [--compact] [--max-symbols N]"));
  assert.ok(output.commandMap.some((entry) => entry.command === "agentshell refs <symbol> [--compact] [--limit N]"));
  assert.ok(output.commandMap.some((entry) => entry.command === "agentshell config list [--compact] [--max-configs N]"));
  assert.ok(output.commandMap.some((entry) => entry.command === "agentshell package script <name> [--compact]"));
...
test("schema list and schema get expose JSON contracts", () => {
  ...
  assert.ok(listOutput.schemas.some((entry) => entry.name === "grep"));
  assert.ok(listOutput.schemas.some((entry) => entry.name === "tree"));
  assert.ok(listOutput.schemas.some((entry) => entry.name === "package-scripts"));
  assert.ok(listOutput.schemas.some((entry) => entry.name === "package-script"));
  assert.ok(listOutput.schemas.some((entry) => entry.name === "package-deps"));
  assert.ok(listOutput.schemas.some((entry) => entry.name === "files-changed"));
  assert.ok(listOutput.schemas.some((entry) => entry.name === "file-info"));
  assert.ok(listOutput.schemas.some((entry) => entry.name === "test-list"));
  assert.ok(listOutput.schemas.some((entry) => entry.name === "errors-from-log"));
  assert.ok(listOutput.schemas.some((entry) => entry.name === "imports"));
  assert.ok(listOutput.schemas.some((entry) => entry.name === "symbols"));
  assert.ok(listOutput.schemas.some((entry) => entry.name === "refs"));
  assert.ok(listOutput.schemas.some((entry) => entry.name === "config-list"));
  assert.ok(listOutput.schemas.some((entry) => entry.name === "git-status"));
  assert.ok(listOutput.schemas.some((entry) => entry.name === "git-diff"));
  assert.ok(listOutput.schemas.some((entry) => entry.name === "git-log"));
  assert.ok(listOutput.schemas.some((entry) => entry.name === "git-branch"));
