...
For local development, uninstall is intentionally manual and reversible:

1. Remove the `agentshell` entry from `~/.agents/plugins/marketplace.json`.
2. Remove `~/plugins/agentshell` if the source copy is no longer needed.
3. Remove stale cache directories under `~/.codex/plugins/cache/personal/agentshell/`.
4. Start a new Codex thread so old skill instructions are not reused from context.

Do not remove this source checkout when cleaning the local plugin copy. The
checkout is the editable source of truth; `~/plugins/agentshell` is generated by
`npm run plugin:install-local`.
