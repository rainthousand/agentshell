...
const POLICY = [
  ...
  "- Resolve the newest installed launcher under `${CODEX_HOME:-$HOME/.codex}/plugins/cache/personal/agentshell/` once per Codex task and prefer its `bin/agentshell`, so an older standalone on PATH cannot shadow the active plugin.",
  "- If no plugin-cache launcher exists, use `./bin/agentshell` from a checkout or `agentshell` from PATH.",
  "- Run the resolved AgentShell launcher with `start --compact` early from that project root.",
